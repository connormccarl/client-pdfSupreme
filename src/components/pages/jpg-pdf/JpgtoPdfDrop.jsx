...
const JpgtoPdfDrop = () => {
  ...
  const handleJpgtoPdf = async () => {
    ...
    // Calling /jpg-pdf
    console.log("Calling /jpg-pdf");

    const apiURL = "https://pdfapi-zdtz.onrender.com";

    // assign form data
    const form = new FormData();
    form.append("page_size_input", "a4");
    form.append("page_orientation", "portrait");
    form.append("margin_input", "small");
    form.append("merge_into_single_pdf", "true");
    form.append("file", uploadedFile);

    // make api call
    const finishedURL = await fetch(apiURL + "/jpg-pdf", {
      method: "POST",
      body: form,
    })
      .then((response) => {
  ...
  useEffect(() => {
    if (mergeComplete) {
      navigate("/updated_jpg_to_pdf", { state: { downloadURL: completedURL } });
    }
  });
  return (
