...
const DropUnlock = () => {
  ...
  const handleUnlock = async () => {
    ...
    // Calling /unlock-pdf
    console.log("Calling /unlock-pdf");

    const apiURL = "https://pdfapi-zdtz.onrender.com";

    // assign form data
    const form = new FormData();
    form.append("unlockpassword", "password");
    form.append("file", uploadedFile);

    // make api call
    const finishedURL = await fetch(apiURL + "/unlock-pdf", {
      method: "POST",
      body: form,
    })
      .then((response) => {
  ...
  useEffect(() => {
    if (mergeComplete) {
      navigate("/unlocked_pdf", { state: { downloadURL: completedURL } });
    }
  });
