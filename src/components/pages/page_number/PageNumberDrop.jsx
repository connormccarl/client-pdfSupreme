...
const PageNumberDrop = () => {
  ...
  const handlePagenumber = async () => {
    ...
    // Calling /pagenumber-pdf
    console.log("Calling /pagenumber-pdf");

    const apiURL = "https://pdfapi-zdtz.onrender.com";

    // assign form data
    const form = new FormData();
    form.append("pagenumber_location", "headerCenter");
    form.append("file", uploadedFile);

    // make api call
    const finishedURL = await fetch(apiURL + "/pagenumber-pdf", {
      method: "POST",
      body: form,
    })
      .then((response) => {
  ...
  useEffect(() => {
    if (mergeComplete) {
      navigate("/updated_pages_pdf", { state: { downloadURL: completedURL } });
    }
  });
  return (
